...
**User install:** [`../hosts/opencode-clones.md`](../hosts/opencode-clones.md) (MiMo/Kilo) · [`../hosts/pi-family.md`](../hosts/pi-family.md) (Pi family) · [`../hosts/dsh-family.md`](../hosts/dsh-family.md) (DSH)
...
**Scope boundary — the Pi family and DSH are outside this contract.** `pi`, `oh-my-pi`, and DeepSeek Harness are not OpenCode forks and ship no `@opencode-ai/plugin`-shaped package, so there is nothing for a facade to delegate to and the `HostProfile`/tier vocabulary below does not apply to them. Unmodified OpenCode `aisdk`-type plugins reach those hosts through **separate** mechanisms — `@opencode-compat/pi-bridge` (`pi.registerProvider(...)`) and `@opencode-compat/dsh-bridge` (`ctx.llm.registerAdapter(...)`). Contracts: [`packages/pi-bridge/README.md`](../../packages/pi-bridge/README.md), [`packages/dsh-bridge/README.md`](../../packages/dsh-bridge/README.md). Do not merge the three.
...
type HostProfile = {
  ...
  capabilities: {
    ...
    /** Finished todo snapshot (no live row) is cleared. true → MiMo and Kilo */
    clearSettledTodos: boolean
    /** empty → MiMo abandon; completed-only → Kilo keeps completed names */
    clearSettledTodoMode: "empty" | "completed-only"
    /** Summed step-finish hosts: occupancy snapshots store zero tokens. true → Kilo */
...
| `clearSettledTodos` | `true` (MiMo, Kilo) | A todo snapshot with no live row leaves the sidebar (§6.6) |
| `clearSettledTodoMode` | `empty` (MiMo) / `completed-only` (Kilo) | Empty abandons MiMo tasks; Kilo keeps completed names so the finish stays visible |
| `collapseOccupancyUsage` | `false` (Kilo) | A provider's declared occupancy snapshots update assistant context; OCP estimates interim step accounting and settles the step sum to declared exact turn counters |
...
Schema-key adoption runs on every host. MiMo also synthesizes `tool-input-start` and fills a missing `bash.description`. MiMo and Kilo clear a finished todo snapshot; OpenCode leaves that snapshot as the model sent it. On Kilo, the shim installs a neutral event capability for any provider that declares `usageVersion: 3` with `occupancyOnly` or the five `*Raw` counters in one provider-metadata namespace. Checkpoint occupancy remains on the assistant message for the context meter. Intermediate step records receive estimates from fresh checkpoints and streamed output; when the terminal raw counters arrive, OCP distributes each exact input/output/cache/reasoning component across the step records so their sum is exact. Generation speed uses step timing and estimated generated tokens until the final output count arrives. Providers without that contract retain their V3 usage unchanged.
...
6. **Translated catalogs preserve full membership in alphabetical canonical order.** Translation never invents or filters tools, and emits them sorted by canonical name (UTF-16 code-unit order, matching cursor-opencode-provider) — not host insertion order — so the same catalog always serializes to the same overlay bytes across turns, restarts, locales, and machines (downstream prompt-cache stability). Schema property keys and `required` arrays are likewise emitted in sorted order; live host state (e.g. agent rosters) is never embedded in tool descriptions.
7. **A finished snapshot clears the visible list on MiMo and Kilo.** No `pending` or `in_progress` row means the list is done. MiMo's sidebar keeps recent `done` tasks and hides `abandoned`, so OCP abandons every known row. Kilo keeps the sidebar up only while some row is not `completed`, so OCP preserves completed rows and drops cancelled rows. This hides the panel without erasing the named finish snapshot. While any row is still live, `completed` still maps to MiMo `done` and Kilo keeps the full array. OpenCode is unchanged.
...
| `@opencode-compat/opencode-loader` | **Outside this contract** (§1): shared host-neutral OpenCode plugin loader used by pi-bridge and dsh-bridge |
| `@opencode-compat/dsh-bridge` | **Outside this contract** (§1): DSH Cordis `LlmAdapter` for unmodified OpenCode `aisdk` plugins. Ships in the same train; shares no runtime with the facade path |

**User-facing default:** `@opencode-compat/ocp` on OpenCode clones; `@opencode-compat/pi-bridge` on the Pi family; `@opencode-compat/dsh-bridge` on DSH. Other `@opencode-compat/*` bridge packages are implementation detail / transitive deps unless an operator pins them deliberately.

**Not shipped:** `@opencode-compat/adapter-{opencode,mimo,kilo,zcode}` — host variance is data inside `profile` + `adapter`, not separate publishable packages. The same rule holds for the Pi family (`PiHostProfile`) and DSH (`DshHostProfile`): one package per family.
...
| 2026-09-01 | Recorded DeepSeek Harness as a third host family served by `@opencode-compat/dsh-bridge` (§1, §11); shared loader `@opencode-compat/opencode-loader` |
