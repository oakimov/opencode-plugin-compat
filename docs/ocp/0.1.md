...
| `collapseOccupancyUsage` | `true` (Kilo) | Intermediate occupancy-only step finishes store zero tokens; terminal context stays on the assistant message and OCP rewrites only the persisted step record to Cursor's exact aggregate counters |
...
Schema-key adoption runs on every host. MiMo also synthesizes `tool-input-start` and fills a missing `bash.description`. MiMo and Kilo clear a finished todo snapshot; OpenCode leaves that snapshot as the model sent it. For an explicitly matched Cursor provider package on Kilo, the shim installs a neutral event capability: checkpoint occupancy remains on the assistant message for the context meter, while Cursor's exact terminal input/output/cache/reasoning counters replace only the persisted step-finish record used by aggregate usage. Intermediate occupancy repeats are zero, so the aggregate is counted once. Other providers retain their own V3 usage unchanged.
