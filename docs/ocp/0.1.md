...
`wirePromiseV2` installs `Symbol.for("opencode.compat.path-bridge")` for the detected host (including pi/omp project dirs `.pi` / `.omp` and agent roots via `PI_CODING_AGENT_DIR` / `PI_CONFIG_DIR`) so unmodified providers resolve CreatePlan / project-config paths without hardcoding `.opencode`.
