...
| `bashDescriptionRequired` | `true` (MiMo) | Fill missing `bash.description` only (never swap host tool catalogs) |

On hosts where both flags already match OpenCode (Kilo today), the shim is still installed for classic `file://…/dist/index.js` loaders but runtime wrap is **identity**.

Shims are **install-tree only** (stock tarball / plugin repo untouched). Re-run `ocp setup` after plugin install/upgrade/reify. Skip with `--no-provider-shim`. Operator notes: [`docs/hosts/mimo.md`](../hosts/mimo.md) §1.1, [`docs/hosts/kilo.md`](../hosts/kilo.md) §1.1.
