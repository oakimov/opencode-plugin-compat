...
- **OpenCode is native — never ocp-dev shim it.** Stock OpenCode (1.x and 2.0) loads consumer plugins through its own plugin surface. Point its `plugin` / provider entry at the published package or the provider checkout `dist/`. `./scripts/ocp-dev.sh run` must not wrap OpenCode, rewrite its config, or install a `~/.cache/ocp-dev/opencode` wrapper. Local-dev shims are for hosts that are *not* OpenCode: MiMo, Kilo, pi, omp, and dsh. Named `run opencode` is an error; default `run` with no host names skips it.
...
- **Consumer plugins** (e.g. `cursor-opencode-provider`) are test/matrix subjects, not deliverables of this repo. Gaps close in the bridge, never by forking the plugin. OpenCode loads those plugins natively; do not route live OpenCode through an ocp-dev wrapper.
